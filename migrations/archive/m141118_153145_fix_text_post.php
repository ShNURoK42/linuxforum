...
class m141118_153145_fix_text_post extends Migration
{
    public function up()
    {
        $time = time() - 1000000000;
        $posts = $this->db->createCommand('SELECT id, text FROM post WHERE created_at > :time');
        $posts->bindParam(':time', $time);
        $posts = $posts->queryAll();
        $count = sizeof($posts);
        $i = 1;

        foreach ($posts as $post) {
            $new_post = $this->bbcode($post['text']);
            $command = $this->db->createCommand('UPDATE post SET text=:text WHERE id=:id');
            $command->bindParam(':id', $post['id']);
            $command->bindParam(':text', $new_post);
            $command->execute();
    ...
    protected function bbcode($bbcode)
    {
        ...
        // [url]
        $bbcode = preg_replace_callback('%\[url\]([\W\D\w\s]*?)\[/url\]%iu',
            function ($matches) {
                return "<" . $matches[1] . ">";
            },
            $bbcode
        ...
        // [url]
        $bbcode = preg_replace_callback('%\[spoiler\]([\W\D\w\s]*?)\[/spoiler\]%iu',
            function ($matches) {
                return $matches[1];
            },
            $bbcode
        );
        $bbcode = preg_replace_callback('%\[spoiler\s*=\s*("(?:[^"]*")|\A[^\']*\Z|(?:[^\'">\]\s]+))\s*(?:[^]\s]*)\]([\W\D\w\s]*?)\[/spoiler\]%iu',
            function ($matches) {
                return $matches[2];
            },
            $bbcode
