...
<div class="pviewforum">
    ...
    <div class="topics-list">
        ...
        <div class="topic-row<?= ($topic->sticked) ? ' sticked' : '' ?><?= ($topic->closed) ? ' closed' : '' ?>">
            <div class="topic-row-icon"><?= ($topic->sticked) ? '<span class="fa fa-thumb-tack"></span>' : '<span class="fa fa-file-text-o "></span>' ?></div>
            ...
            <div class="<?= ($topic->number_posts == 0) ? 'topic-row-post-null' : 'topic-row-post-count' ?>"><?= Yii::$app->formatter->asInteger($topic->number_posts) ?> <span class="fa fa-comments"></span>
            </div>
        </div>
        <?php endforeach; ?>
    </div>
