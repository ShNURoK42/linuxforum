    ...
    <div class="box">
        <div class="inbox">
            <table>
                ...
                <tr class="<?= ($item['topic_count'] % 2 == 0) ? 'roweven' : 'rowodd' ?><?= ($topic->sticky) ? ' isticky' : ''?><?= ($topic->closed) ? ' iclosed' : ''?>">
                    <td class="tcl">
                        <div class="icon"><div class="nosize">1</div></div>
                        <div class="tclcon">
                            <div>
                                <?php if ($topic->moved_to):?>
                                <span class="movedtext">Перенесено:</span>
                                <a href="<?= Url::toRoute(['topic/view', 'id' => $topic->moved_to])?>"><?= $formatter->asText($topic->subject) ?></a>
                                <?php else: ?>
                                    <?php if ($topic->sticky): ?>
                                ...
                                <?php endif; ?>
                                <span class="byuser"><?= $topic->poster ?></span>
                                <?= TopicPager::widget(['topic' => $topic]) ?>
                            </div>
                        </div>
                    </td>
                    <td class="tc2"><?= Yii::$app->formatter->asInteger($topic->num_replies) ?></td>
                    <td class="tc3"><?= Yii::$app->formatter->asInteger($topic->num_views) ?></td>
                    <td class="tcr">
                        <a href="<?= Url::toRoute(['post/view', 'id' => $topic->last_post_id, '#' => 'p' . $topic->last_post_id ]) ?>"><?= Yii::$app->formatter->asDatetime($topic->last_post) ?></a>
                        <span class="byuser"><?= $formatter->asText($topic->last_poster) ?></span>
                    </td>
                </tr>
                <?php endforeach; ?>
                <?php endif; ?>
